...
import { MobileCurrentDashboard } from '../components/MobileCurrentDashboard'
...
const mobileNavItems: { id: MobileTab; label: string; icon: string }[] = [
  { id: 'current', label: 'Home', icon: 'H' },
  { id: 'chat', label: 'Moon', icon: 'M' },
  { id: 'planning', label: 'Plan', icon: 'P' },
  { id: 'journal', label: 'Journal', icon: 'J' },
...
function Dashboard() {
  ...
  const mobileTitle = mobileNavItems.find((item) => item.id === mobileTab)?.label || 'Current'
  const nextFullMoon =
    moonContext?.next_full_moon ||
    findPhaseEvent(moonCalendar?.days, 'full')?.date
  const nextNewMoon =
    moonContext?.next_new_moon ||
    findPhaseEvent(moonCalendar?.days, 'new')?.date
  ...
  useEffect(() => {
    if (
      (activeSection === 'journal' || mobileTab === 'journal') &&
      journalEntries.length === 0
    ) {
      void loadJournalEntries()
    }
  }, [activeSection, journalEntries.length, mobileTab])

  useEffect(() => {
    if (activeSection === 'calendar' || mobileTab === 'current') {
      void loadMoonCalendar(calendarDate)
    }
  }, [activeSection, calendarDate, mobileTab])
  ...
  function renderChatPanel(className = 'chat-panel') {
    return (
      <ChatPanel
        className={className}
        error={error}
  ...
  function renderMobileSettings() {
    return (
      <section className="mobile-simple-screen">
        <div className="panel">
          <h2>Settings</h2>
          <p>
            App preferences are coming soon. For now, use the desktop view for
            full configuration and connection details.
          </p>
        </div>
      </section>
    )
  ...
  function renderMobileScreen() {
    if (mobileTab === 'current') return renderMobileCurrent()
    if (mobileTab === 'chat') return renderChatPanel('chat-panel mobile-chat-panel')
    if (mobileTab === 'planning') return renderPlanning()
    if (mobileTab === 'journal') return renderJournal()
  ...
  return (
    <>
      <main className="app-shell">
        ...
        <RightRail moonContext={moonContext} moonError={moonError} phase={phase} />
      </main>
      {renderMobileShell()}
    </>
  )
